...
class correlation_tool_Input(BaseModel):
    ...
    excluded: list[str] = Field(
        default=[],
        description="List of column names to exclude from correlation analysis (e.g. free text, ID columns , continous features). Continous features will be one-hot encoded if not excluded in here"
    )
...
class one_hot_encoding_tool_Input(BaseModel):
    ...
    columns_drop: List[str] = Field(
        default=None,
        description="All the continous features idenfied"
    )
...
class drop_columns_tool(BaseTool):
    ...
    def _run(self, path: str, columns_to_drop: List[str]) -> str:
        ...
        timestamp = now.strftime("%Y%m%d_%H%M%S")
        output_path = path_name.parent / f"{path_name.stem}_dropped_{timestamp}.csv"
...
class correlation_tool(BaseTool):
    ...
    description: str = (
        "Performs correlation to identify the correlation between a target feature "
...
if __name__ == "__main__":
    path = r"C:\Users\tvlan\Downloads\early_sepsis_full_simulated_dataset.csv"

    tool = one_hot_encoding_tool()
   ...
   "shock_index",                                                                                                                             
  "high_risk_symptom_count", "text_symptom_count"],
        target ="sepsis_risk")
